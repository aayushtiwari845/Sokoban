...
validation loss moves by only $\pm 0.005$, so validation loss is a poor proxy
for the property being evaluated and single-seed comparisons between nearby rows
should not be read as settled. Repeating the pretraining ablation three times
under the same protocol does settle it: a DistilGPT-2 body with a
re-initialised $48$-symbol vocabulary reaches $64.2\% \pm 1.8$ against
$42.7\% \pm 7.0$, with no overlap between the two sets of runs. Pretrained
sequence-modelling machinery transfers even when no vocabulary does --- and, on
this evidence, makes training markedly more stable.
